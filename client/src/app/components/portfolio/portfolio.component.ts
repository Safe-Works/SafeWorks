...
export class PortfolioComponent implements OnInit {
  ...
  async createPortfolio() {
    ...
    await this.portfolioService
      ...
      .catch((error) => {
        this.openSnackBar(
          "Ocorreu um erro ao criar o portfolio!",
          "OK",
          "snackbar-error"
        );
        console.error(error);
  ...
  async addCertification() {
    ...
    const certification: Certification = {
      ...
      description: this.certificationForm.get("certificationDescription")
        ?.value,
      issue_organization: this.certificationForm.get(
        "certificationIssueOrganization"
      )?.value,
      issue_date: new Date(
        this.certificationForm.get("certificationIssueDate")?.value
      ),
      certification_url: this.certificationForm.get("certificationUrl")?.value,
    };

    await this.portfolioService
      .AddCertification(portfolioUid, certification)
      .then((response) => {
      ...
      .catch((error) => {
        this.openSnackBar(
          "Ocorreu um erro ao adicionar uma certificação!",
          "OK",
          "snackbar-error"
        );
        console.error(error);
  ...
  async updatePortfolio() {
    ...
    await this.portfolioService
      ...
      .catch((error) => {
        this.openSnackBar(
          "Ocorreu um erro para ao atualizar o portfolio!",
          "OK",
          "snackbar-error"
        );
        console.error(error);
  ...
  async getPortfolio() {
    ...
    await this.userService
      .GetUserInfo(this.userAuth.currentUser?.uid ?? "")
      .then((response) => {
        this.portfolioUid = response.user.worker.portfolio;
      })
      .catch((error) => {
        this.openSnackBar(
          "Ocorreu um erro para obter o portfolio!",
          "OK",
          "snackbar-error"
        );
        console.error(error);
    ...
    if (this.portfolioUid) {
      await this.portfolioService
        .GetPortfolio(this.portfolioUid)
        .then((response) => {
          if (response) {
            const portfolio = response.portfolio;
            this.description.setValue(portfolio.description);
  ...
  public async deleteCertification(certification: Certification) {
    ...
    try {
      await this.portfolioService
        .DeleteCertification(certificationId.slice(0, 20), certificationId)
        .then((response) => {
          this.openSnackBar(
            "Certificação excluída com sucesso!",
            "OK",
            "snackbar-success"
          );
          this.certifications = response.portfolio.certifications;
        ...
        .catch((error) => {
          this.openSnackBar(
            "Ocorreu um erro ao excluir uma certificação!",
            "OK",
            "snackbar-error"
          );
          console.error(error);
